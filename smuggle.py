...
if __name__ == '__main__':
    cp = ColorPallete()
    request_headers = {}
    parser = argparse.ArgumentParser(description="Send (malformed) http requests to a host.\n\nExample: python3 smuggle.py --host stallman.org --request-headers \"Transfer-Encoding:chunked,Content-Type:text/html\" --smuggled-body-fields \"\" --body-data \"\" --port 443 --debug 1")
    ...
    for i in range(len(args_map)):
        ...
        if k == 'debug' and v == None:
            ...
            args.debug = False if v in ['False','false','0','off','none'] else v in ['True','true','1','on','all'] 
